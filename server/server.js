...
const uri =
  "mongodb://SmartRent:berlin28041993@smartrentdb-shard-00-00-estgp.mongodb.net:27017,smartrentdb-shard-00-01-estgp.mongodb.net:27017,smartrentdb-shard-00-02-estgp.mongodb.net:27017/test?ssl=true&replicaSet=SmartRentDB-shard-0&authSource=admin&retryWrites=true";
const connectionErrorMessage =
  "Connection with the database could not be establised.";
    ...
    function(err, db) {
      ...
      dbo.collection("accounts").findOne(query, function(err, document) {
        if (err) {
          response.send({
            success: false,
            message: "Error by querying accounts."
          });
          throw err;
        } else {
      ...
      dbo
        ...
        .toArray(function(err, documents) {
          if (err) {
            ...
            response.send({
              success: true,
              accounts: JSON.stringify(documents)
            });
          }
        });
      db.close();
      ...
      dbo
        ...
        .toArray(function(err, documents) {
          if (err) {
            response.send({
              success: false,
              message: "Error by querying transactions."
            });
            throw err;
          } else {
            response.send({
              success: true,
              transactions: JSON.stringify(documents)
            });
          }
        });
      db.close();
...
app.post("/api/createUser", (request, response) => {
  mongoClient.connect(
    ...
    function(err, db) {
      ...
      dbo
        .collection("accounts")
        .findOneAndUpdate(query, update, { returnOriginal: false }, function(
          err,
          document
        ) {
          if (err) {
      ...
      dbo
        ...
        .findOneAndUpdate(query, update, { returnOriginal: false }, function(
          ...
          if (err) {
            ...
            var apartment =
              document.value.apartments[document.value.apartments.length - 1];
            response.send({
              success: true,
    ...
    (err, db) => {
      ...
      dbo.collection("accounts").insertMany(req.body, (err, result) => {
        if (err) {
          ...
          response.send({
            success: true,
            message: result.insertedCount + " accounts created."
          });
        }
      });
      db.close();
